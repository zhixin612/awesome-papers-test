The paper list will be updated automatically, please do not edit.

### 2025-02-10

* [EcoServe: Designing Carbon-Aware AI Inference Systems](https://arxiv.org/abs/2502.05043)
* [Optimisation of ATLAS computing resource usage through a modern HEP Benchmark Suite via HammerCloud and Big PanDA](https://arxiv.org/abs/2502.04853)
* [Survey on Token-Based Distributed MutualExclusion Algorithms](https://arxiv.org/abs/2502.04708)
* [A Randomised Approach to Distributed Sorting](https://arxiv.org/abs/2502.05082)
* [Federated Learning for Anomaly Detection in Energy Consumption Data: Assessing the Vulnerability to Adversarial Attacks](https://arxiv.org/abs/2502.05041)
* [Aequa: Fair Model Rewards in Collaborative Learning via Slimmable Networks](https://arxiv.org/abs/2502.04850)
* [WaferLLM: A Wafer-Scale LLM Inference System](https://arxiv.org/abs/2502.04563)

### 2025-02-07

* [FedOptimus: Optimizing Vertical Federated Learning for Scalability and Efficiency](https://arxiv.org/abs/2502.04243)
* [PSZ: Enhancing the SZ Scientific Lossy Compressor With Progressive Data Retrieval](https://arxiv.org/abs/2502.04093)
* [Oblivious Robots Under Round Robin: Gathering on Rings](https://arxiv.org/abs/2502.03939)
* [Exploring Uncore Frequency Scaling for Heterogeneous Computing](https://arxiv.org/abs/2502.03796)
* [HACK: Homomorphic Acceleration via Compression of the Key-Value Cache for Disaggregated LLM Inference](https://arxiv.org/abs/2502.03589)
* [Non-convex composite federated learning with heterogeneous data](https://arxiv.org/abs/2502.03958)
* [InfinitePOD: Building Datacenter-Scale High-Bandwidth Domain for LLM with Optical Circuit Switching Transceivers](https://arxiv.org/abs/2502.03885)
### 2025-02-06

* [STEM: Spatial-Temporal Mapping Tool For Spiking Neural Networks](https://arxiv.org/abs/2502.03287)
* [Data Dams: A Novel Framework for Regulating and Managing Data Flow in Large-Scale Systems](https://arxiv.org/abs/2502.03218)
* [Multi-objective methods in Federated Learning: A survey and taxonomy](https://arxiv.org/abs/2502.03108)
* [E-3SFC: Communication-Efficient Federated Learning with Double-way Features Synthesizing](https://arxiv.org/abs/2502.03092)
* [Vertical Federated Learning for Failure-Cause Identification in Disaggregated Microwave Networks](https://arxiv.org/abs/2502.02874)
* [A Study on 5G Network Slice Isolation Based on Native Cloud and Edge Computing Tools](https://arxiv.org/abs/2502.02842)
* [Federated Low-Rank Tensor Estimation for Multimodal Image Reconstruction](https://arxiv.org/abs/2502.02761)
* [Parameter Tracking in Federated Learning with Adaptive Optimization](https://arxiv.org/abs/2502.02727)
### 2025-02-05

* [Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism](https://arxiv.org/abs/2502.02581)
* [H-MBR: Hypervisor-level Memory Bandwidth Reservation for Mixed Criticality Systems](https://arxiv.org/abs/2502.02437)
* [An inherently parallel H2-ULV factorization for solving dense linear systems on GPUs](https://arxiv.org/abs/2502.02395)
* [Efficient Extensions for Asynchronous Byzantine Agreement via Weak Agreement](https://arxiv.org/abs/2502.02320)
* [Optimal Broadcast on Congested Random Graphs](https://arxiv.org/abs/2502.02165)
* [Evaluating Fault Tolerance and Scalability in Distributed File Systems: A Case Study of GFS, HDFS, and MinIO](https://arxiv.org/abs/2502.01981)
* [A Multi-Objective Framework for Optimizing GPU-Enabled VM Placement in Cloud Data Centers with Multi-Instance GPU Technology](https://arxiv.org/abs/2502.01909)
* [LV-XAttn: Distributed Cross-Attention for Long Visual Inputs in Multimodal Large Language Models](https://arxiv.org/abs/2502.02406)
* [FPGA Innovation Research in the Netherlands: Present Landscape and Future Outlook](https://arxiv.org/abs/2502.02404)
* [Random Adaptive Cache Placement Policy](https://arxiv.org/abs/2502.02349)
* [Comparative Analysis of FPGA and GPU Performance for Machine Learning-Based Track Reconstruction at LHCb](https://arxiv.org/abs/2502.02304)
* [SMTFL: Secure Model Training to Untrusted Participants in Federated Learning](https://arxiv.org/abs/2502.02038)
* [Ilargi: a GPU Compatible Factorized ML Model Training Framework](https://arxiv.org/abs/2502.01985)
* [Optimizing Spot Instance Reliability and Security Using Cloud-Native Data and Tools](https://arxiv.org/abs/2502.01966)
* [Longer Attention Span: Increasing Transformer Context Length with Sparse Graph Processing Techniques](https://arxiv.org/abs/2502.01659)
* [How to Build a Quantum Supercomputer: Scaling Challenges and Opportunities](https://arxiv.org/abs/2411.10406)
### 2025-02-04

* [Reductions in local certification](https://arxiv.org/abs/2502.01551)
* [SQUASH: Serverless and Distributed Quantization-based Attributed Vector Similarity Search](https://arxiv.org/abs/2502.01528)
* [OCTOPINF: Workload-Aware Inference Serving for Edge Video Analytics](https://arxiv.org/abs/2502.01277)
* [Near-State and State-Optimal Self-Stabilising Leader Election Population Protocols](https://arxiv.org/abs/2502.01227)
* [Self-Organizing Interaction Spaces: A Framework for Engineering Pervasive Applications in Mobile and Distributed Environments](https://arxiv.org/abs/2502.01137)
* [Deep Reinforcement Learning for Dynamic Resource Allocation in Wireless Networks](https://arxiv.org/abs/2502.01129)
* [Towards Efficient Large Multimodal Model Serving](https://arxiv.org/abs/2502.00937)
* [Demystifying Cost-Efficiency in LLM Serving over Heterogeneous GPUs](https://arxiv.org/abs/2502.00722)
* [DeLIAP e DeLIAJ: Interfaces de biblioteca de Dependabilidade para Python e Julia](https://arxiv.org/abs/2502.00703)
* [Optimal local certification on graphs of bounded pathwidth](https://arxiv.org/abs/2502.00676)
* [General Coded Computing in a Probabilistic Straggler Regime](https://arxiv.org/abs/2502.00645)
* [GPU-Accelerated Modified Bessel Function of the Second Kind for Gaussian Processes](https://arxiv.org/abs/2502.00356)
* [Alternative Mixed Integer Linear Programming Optimization for Joint Job Scheduling and Data Allocation in Grid Computing](https://arxiv.org/abs/2502.00261)
* [Bounding Distance Between Outputs in Distributed Lattice Agreement](https://arxiv.org/abs/2502.00247)
* [Asynchronous Fault-Tolerant Language Decidability for Runtime Verification of Distributed Systems](https://arxiv.org/abs/2502.00191)
* [Dismountability in Temporal Cliques Revisited](https://arxiv.org/abs/2502.01321)
* [A Framework for Double-Blind Federated Adaptation of Foundation Models](https://arxiv.org/abs/2502.01289)
* [Leveraging InfiniBand Controller to Configure Deadlock-Free Routing Engines for Dragonflies](https://arxiv.org/abs/2502.01214)
* [FedRIR: Rethinking Information Representation in Federated Learning](https://arxiv.org/abs/2502.00859)
* [ATA: Adaptive Task Allocation for Efficient Resource Management in Distributed Machine Learning](https://arxiv.org/abs/2502.00775)
* [POSMAC: Powering Up In-Network AR/CG Traffic Classification with Online Learning](https://arxiv.org/abs/2502.00671)
* [Work-Efficient Parallel Non-Maximum Suppression Kernels](https://arxiv.org/abs/2502.00535)
* [A Novel Approach to Translate Structural Aggregation Queries to MapReduce Code](https://arxiv.org/abs/2502.00343)
* [Enhancing Token Filtering Efficiency in Large Language Model Training with Collider](https://arxiv.org/abs/2502.00340)
* [Physics-Inspired Distributed Radio Map Estimation](https://arxiv.org/abs/2502.00319)


